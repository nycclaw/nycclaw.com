# Integration & Vertical Pages — Build Tracker

## Updated: 2026-03-21
## Strategy doc: `research/integration-expansion-q2-2026.md`

### Legend
- ⬜ pending | 🔧 needs fix | 🔨 building | 🔍 review | ✅ live | ❌ failed/redo

---

## ⏸️ Week 1: CTR Fixes (Existing Pages — Title/Meta Rewrites) — ON HOLD
> Parked — not sure this is the right move yet. Revisit later.

| # | Page | Position | Impressions (30d) | Clicks | Status |
|---|------|----------|-------------------|--------|--------|
| 1 | Wave | 3.6 | 35 | 0 | 🔧 |
| 2 | HawkSoft | 1.4 | 9 | 0 | 🔧 |
| 3 | Housecall Pro | 2.1 | 22 | 0 | 🔧 |
| 4 | QuickBooks | 4.1 | 24 | 0 | 🔧 |
| 5 | Guesty | 5.0 | 33 | 0 | 🔧 |
| 6 | GoHighLevel | 8.0 | 588 | 3 | 🔧 |
| 7 | Athenahealth | 6.8 | 68 | 0 | 🔧 |
| 8 | Buildium | 21.0 | 64 | 0 | 🔧 |
| 9 | kvCORE | 16.7 | 31 | 0 | 🔧 |
| 10 | Lofty | 19.2 | 11 | 0 | 🔧 (full rewrite — see Week 2) |

---

## 🔴 Week 2: Tier 1A — Dental + Financial Advisor + Lofty

### New Integration Pages

| # | Platform | Category | Slug | Status |
|---|----------|----------|------|--------|
| 11 | Dentrix | Dental PM | dentrix | ✅ deployed Mar 21 |
| 12 | Eaglesoft | Dental PM | eaglesoft | ✅ deployed Mar 21 |
| 13 | Open Dental | Dental PM | open-dental | ✅ deployed Mar 21 |
| 14 | Redtail CRM | Financial CRM | redtail | ✅ deployed Mar 21 |
| 15 | Wealthbox | Financial CRM | wealthbox | ✅ deployed Mar 21 |
| 16 | Riskalyze/Nitrogen | Financial Tools | riskalyze | ✅ deployed Mar 21 |

### Content Rewrites (Completed Mar 21)

| # | Platform | Action | Status |
|---|----------|--------|--------|
| 17 | **Lofty** | Full rewrite: Smart Plans, IDX, lead routing, Lofty AI vs OpenClaw | ✅ deployed Mar 21 |
| 18 | **GoHighLevel** | Full rewrite: orchestration-layer framing, Workflows v2, sub-accounts, SaaS Mode | ✅ deployed Mar 21 |
| 19 | **kvCORE** | Full rewrite: speed-to-lead, lead triage, behavioral triggers | ✅ deployed Mar 21 |
| 20 | **Buildium** | Full rewrite: PM workflows, maintenance triage, lease renewal, owner reporting | ✅ deployed Mar 21 |

### Vertical Page Updates

| # | Page | Action | Status |
|---|------|--------|--------|
| — | Integrations index | Added 6 new tools to directory | ✅ Mar 21 |
| — | `/for/personal-injury` | NEW: "AI for PI lawyers" (350/mo, KD 2). Medical records, demand letters, intake, case valuation, lien tracking, mass tort. Cross-linked to 7 legal integration pages. | ✅ deployed Mar 21 |
| — | `/for/law-firms` | Added PI cross-link | ✅ Mar 21 |
| — | Filevine | Added PI-specific content + cross-link to /for/personal-injury | ✅ Mar 21 |
| — | Litify | Added PI-specific content + cross-link to /for/personal-injury | ✅ Mar 21 |
| — | Clio, MyCase, PracticePanther, Smokeball | Added PI cross-links | ✅ Mar 21 |
| — | `/for/dental-practices` | Add links to Dentrix, Eaglesoft, Open Dental | ⬜ |
| — | `/for/financial-advisors` | Add links to Redtail, Wealthbox, Riskalyze | ⬜ |
| — | `/for/real-estate` | Add prominent Lofty link | ⬜ |

---

## 🔴 Week 3: Tier 1B — Trades + Messaging + Core Tools

### New Integration Pages

| # | Platform | Category | Slug | Status |
|---|----------|----------|------|--------|
| 18 | FieldEdge | Home Services | fieldedge | ⬜ |
| 19 | ServiceM8 | Home Services | servicem8 | ⬜ |
| 20 | Vonigo | Home Services | vonigo | ⬜ |
| 21 | JobNimbus | Roofing/Contractors | jobnimbus | ⬜ |
| 22 | Telegram | Messaging | telegram | ⬜ |
| 23 | WhatsApp | Messaging | whatsapp | ⬜ |
| 24 | Microsoft Teams | Messaging | microsoft-teams | ⬜ |
| 25 | Stripe | Payments | stripe | ⬜ |
| 26 | Shopify | E-commerce | shopify | ⬜ |
| 27 | Google Workspace | Productivity | google-workspace | ⬜ |
| 28 | Airtable | Database/PM | airtable | ⬜ |

### New Vertical Page

| # | Page | Slug | Status |
|---|------|------|--------|
| 29 | Contractors | `/for/contractors` | ⬜ |

---

## 🟡 Week 4: Tier 1C — Content & Hub Pages

| # | Page | Target Keyword | Vol | Slug | Status |
|---|------|---------------|-----|------|--------|
| 30 | Claude for Business | "claude for business" | 250 | claude-for-business | ⬜ |
| 31 | MCP Integrations | "claude mcp integrations" | 60 | integrations/mcp | ⬜ |
| 32 | AI Scheduling Guide | "ai scheduling assistant" | 1,700 | guides/ai-scheduling | ⬜ |
| 33 | AI for Small Business | "ai assistant for small business" | 90 | guides/ai-small-business | ⬜ |

---

## 🟡 Weeks 5-6: Tier 2 — Vet + PT/Chiro + CRMs

### New Integration Pages

| # | Platform | Category | Slug | Status |
|---|----------|----------|------|--------|
| 34 | eVetPractice | Veterinary | evetpractice | ⬜ |
| 35 | Shepherd | Veterinary | shepherd | ⬜ |
| 36 | Cornerstone (IDEXX) | Veterinary | cornerstone | ⬜ |
| 37 | Jane App | PT/Wellness | jane-app | ⬜ |
| 38 | ChiroTouch | Chiropractic | chirotouch | ⬜ |
| 39 | WebPT | Physical Therapy | webpt | ⬜ |
| 40 | Mindbody | Fitness/Wellness | mindbody | ⬜ |
| 41 | Keap | SMB CRM | keap | ⬜ |
| 42 | Copper | Google CRM | copper | ⬜ |
| 43 | ActiveCampaign | Marketing CRM | activecampaign | ⬜ |

### New Vertical Pages

| # | Page | Slug | Status |
|---|------|------|--------|
| 44 | Veterinary Clinics | `/for/veterinary-clinics` | ⬜ |
| 45 | Physical Therapy | `/for/physical-therapy` | ⬜ |
| 46 | Chiropractors | `/for/chiropractors` | ⬜ |

---

## Previously Built (Live — 51 integration + 16 vertical pages)

### Integration Pages ✅
apollo, appfolio, applied-epic, athenahealth, attio, bench, buildium, calendly, clay, clio, close, cosmolex, dealmachine, drchrono, filevine, followupboss, freshbooks, freshsales, gohighlevel, greenhouse, guesty, hawksoft, housecallpro, hubspot, instantly, jobber, kareo, kvcore, lindy, liondesk, litify, lofty, mailchimp, make, monday, mycase, n8n, notion, pipedrive, practicepanther, quickbooks, rocketmatter, salesforce, servicetitan, slack, smokeball, wave, xero, zapier, zendesk, zoho

### Vertical Pages ✅
accounting-firms, architecture-firms, coaches-consultants, creative-agencies, dental-practices, ecommerce, financial-advisors, healthcare, insurance-agencies, law-firms, mortgage-brokers, personal-injury, property-management, real-estate, real-estate-investors, recruiting-agencies, startup-founders

---

## Totals

| Category | Current | Adding | Target |
|----------|---------|--------|--------|
| Integration pages | 51 | 24 | 75 |
| Vertical pages | 17 | 5 | 22 |
| Guide/hub pages | ~8 | 4 | 12 |
| **All pages** | **~110** | **~27** | **~137** |

---

## Batch Log

| Batch | Pages | Started | Completed | Notes |
|-------|-------|---------|-----------|-------|
| CTR Fix | #1-10 (title/meta rewrites) | | | |
| Tier 1A | #11-17 (dental + FA + Lofty) | | | |
| Tier 1B | #18-29 (trades + msg + core) | | | |
| Tier 1C | #30-33 (guides/hubs) | | | |
| Tier 2 | #34-46 (vet + PT + CRMs) | | | |

_Updated: 2026-03-21_
